/**
 * RoomBooking.jsx — Fork & Flame private room booking page
 * -----------------------------------------------------------
 * npm install gsap
 * Requires Tailwind CSS already set up in the project.
 *
 * Fonts: add these once, globally (e.g. app/layout.jsx or globals.css) —
 * this component just references the family names via Tailwind's
 * arbitrary font-family syntax, it doesn't load fonts itself.
 *
 *   import { Fraunces, Inter } from 'next/font/google';
 *   const fraunces = Fraunces({ subsets: ['latin'], variable: '--font-display' });
 *   const inter = Inter({ subsets: ['latin'], variable: '--font-body' });
 *   // then on <html className={`${fraunces.variable} ${inter.variable}`}>
 *
 * No next/font? Just drop this in <head>:
 *   <link href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,500;9..144,600&family=Inter:wght@400;500;600&display=swap" rel="stylesheet" />
 *
 * Colors are inlined as Tailwind arbitrary values (bg-[#17110D] etc.) so
 * this drops into any project without touching tailwind.config — pull
 * them into your theme.extend.colors later if you want shorthand names.
 *
 * Drop <RoomBooking /> in as a page or route component. All animation
 * logic is scoped with gsap.context so it cleans up on unmount.
 */
...
export default function RoomBooking() {
  ...
  // ---------- Hero ignite + ember particles (the one load-time moment) ----------
  useLayoutEffect(() => {
    const reduceMotion =
    ...
    const ctx = gsap.context(() => {
      ...
      // Drifting embers behind the hero — subtle, continuous, low-key
  ...
  // ---------- Alternating room panels: directional wipe on scroll ----------
  useEffect(() => {
    const reduceMotion =
      typeof window !== 'undefined' &&
      window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    ...
    >
      ...
      <section className="relative min-h-[92vh] flex flex-col items-center justify-center text-center px-6 py-24 border-b border-white/10">
        ...
        <p className="rb-subtitle relative z-10 mt-6 max-w-[42ch] text-[#F4EFE6]/75 text-[1.05rem] leading-relaxed">
          ...
          below, or tell us the size of your party and we'll match you to a
          room.
        </p>
      ...
      <section className="py-16">
        {ROOMS.map((room, i) => (
          ...
          >
            <div
              className={`rb-panel-image relative min-h-[340px] bg-cover bg-center after:content-[''] after:absolute after:inset-0 after:bg-gradient-to-t after:from-black/25 after:to-transparent after:pointer-events-none ${
                i % 2 === 1 ? 'md:order-2' : ''
              }`}
        ...
        >
          ...
          {status === 'reserved' && (
            ...
            >
              ...
              {form.date} at {form.time}. We'll email {form.name} a confirmation
              shortly.
            </div>
          )}
        </form>
